...
def generate_derived_plots(sat_results: Dict[str, Any], sat_id: str, output_path: str, dpi: int = 150) -> bool:
    ...
    if bending_csv and os.path.exists(bending_csv):
        ...
        if 'bending_L1' in df.columns:
            ax.plot(df['bending_L1'] * 1e3, df['tangent_height_km'], 'b-', linewidth=1.5, label='L1', alpha=0.8)
        if 'bending_L2' in df.columns:
            ax.plot(df['bending_L2'] * 1e3, df['tangent_height_km'], 'r--', linewidth=1.5, label='L2', alpha=0.8)
        if 'bending_angle_rad' in df.columns:
            ax.plot(df['bending_angle_rad'] * 1e3, df['tangent_height_km'], 'g-', linewidth=2, label='Iono-free', alpha=0.9)
        ax.legend(fontsize=9, loc='upper right')
    ax.set_xlabel('Bending Angle (mrad)'); ax.set_ylabel('Tangent Height (km)')
    ax.set_title('(a) Bending Angle Profile', fontsize=11, fontweight='bold'); ax.grid(True, alpha=0.3); ax.set_xlim(left=0)
    ...
    if comp_csv and os.path.exists(comp_csv):
        ...
        ax.legend(fontsize=9, loc='upper right')
    ax.set_xlabel('Refractivity (N-units)'); ax.set_ylabel('Height (km)')
    ax.set_title('(b) Refractivity Profile', fontsize=11, fontweight='bold'); ax.grid(True, alpha=0.3); ax.set_xlim(left=0)
    ...
    ax.set_xlabel('Pressure (hPa)'); ax.set_ylabel('Height (km)')
    ax.set_title('(c) Pressure Profiles (P, Pw)', fontsize=11, fontweight='bold'); ax.grid(True, alpha=0.3)

    ax = axes[1, 1]
    if atm_csv and os.path.exists(atm_csv):
        df = pd.read_csv(atm_csv)
        if 'T_era5' in df.columns:
            ax.plot(df['T_era5'], df['height_km'], 'g-', linewidth=1.8, label='T (ERA5)')
            ax.legend(fontsize=9, loc='upper right')
        else:
            ax.text(0.5, 0.5, 'Temperature data\nnot available', ha='center', va='center', transform=ax.transAxes, fontsize=12, color='gray')
    else:
        ax.text(0.5, 0.5, 'Requires ERA5 data', ha='center', va='center', transform=ax.transAxes, fontsize=12, color='gray')
    ax.set_xlabel('Temperature (K)'); ax.set_ylabel('Height (km)')
    ax.set_title('(d) Temperature Profile', fontsize=11, fontweight='bold'); ax.grid(True, alpha=0.3)
